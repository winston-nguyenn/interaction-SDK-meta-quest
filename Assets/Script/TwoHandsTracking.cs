...
using Unity.Services.Core;
 ...
 public class TwoHandsTracking : MonoBehaviour
 {
    ...
    async void Start()
    {
        ...
        // Add header
        poseDataList.Add("Timestamp (yyyyMMdd_HHmmss.ffff),Hand,Joint,PositionX,PositionY,PositionZ,RotationX,RotationY,RotationZ,RotationW");

        // Initialize Start time
        startTime = DateTime.Now;
    }
    // Update is called every frame (the game frame rate is typically from 30 to 60 frames per second)
    void Update()
    {
        // Track left hand joints
        TrackHandJoints(leftHand, "Left");

        // Track right hand joints
        TrackHandJoints(rightHand, "Right");
    ...
    void TrackHandJoints(Hand hand, string handLabel)
    {
        foreach (HandJointId jointId in handJointIds)
        {
            if (hand.GetJointPose(jointId, out currentPose))
            {
                SavePoseToString(handLabel, jointId, currentPose);
            }
        }
    }

    void SavePoseToString(string handLabel, HandJointId jointId, Pose pose)
    {
        ...
        string poseData = $"{timestamp},{handLabel},{jointId},{pose.position.x},{pose.position.y},{pose.position.z}," +
